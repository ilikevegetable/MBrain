...
from criterion.downstream_task_criterion import LinearClassifier4EEG, DownstreamCriterion
...
class metric_index():
    def __init__(self, micro_pre, micro_rec, micro_f1, micro_f_d, auc):
...
def print_metrics(true_label, pred_label, soft_label):
    ...
    f2 = fbeta_score(true_label, pred_label, beta=2.0, average='binary')
    auc = roc_auc_score(true_label, soft_label)
    print("-" * 40)
    print("accuracy:", acc)
    print("precision:", pre)
    print("recall:", rec)
    ...
    print("auc score:", auc)
    print("-" * 40)
    index = metric_index(pre, rec, f1, f2, auc)
    return index
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ssl_dir', type=str, default='/data/EEG_checkpoints/',
                        help='The path of SSL model to load.')
    parser.add_argument('--save_dir', type=str, default='/data/EEG_downstream_checkpoints/',
    ...
    for repeat_idx in range(args.repeat_time):
        ...
        downstream_classifier = LinearClassifier4EEG(
