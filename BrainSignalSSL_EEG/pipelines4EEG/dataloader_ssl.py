...
class SeizureDataset(Dataset):
    def __init__(
        ...
        if split == 'train':
            self.data = np.load(os.path.join(self.save_dir, 'ssl_train_data.npz'))['a']
            self.data = torch.tensor(self.data, dtype=torch.float)
        elif split == 'dev':
            self.data = np.load(os.path.join(self.save_dir, 'ssl_val_data.npz'))['a']
            self.data = torch.tensor(self.data, dtype=torch.float)
    ...
    def __getitem__(self, idx):
        ...
        assert out_data.size(0) == self.input_len
