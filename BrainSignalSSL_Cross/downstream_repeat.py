...
from criterion.downstream_task_criterion import LinearClassifier, LinearClassifier4test, NewDownstreamCriterion
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ssl_dir', type=str, default='/data/caidonghong/nips22/MBrain_02GJX_0508/',
                        help='The path of SSL model to load.')
    parser.add_argument('--save_dir', type=str, default='/data/caidonghong/nips22/rebuttal/ablation/downstream/01TGX_random/',
                        help='The path for saving checkpoint.')
    # load dataset setting
    parser.add_argument('--train_patients', type=str, nargs='+', default=['01TGX', '02GJX', '03ZXY'],
                        help='Datasets with more than one patient for pre-training.')
    parser.add_argument('--valid_patient', type=str, default='05ZLH',
                        help='Datasets with more than one patient for pre-training.')
    parser.add_argument('--test_patient', type=str, default='06ZYJ',
                        help='Datasets with more than one patient for pre-training.')

    parser.add_argument('--database_save_dir', type=str, default='/data/caidonghong/new_database2_no_filter/',
    ...
    for repeat_idx in range(args.repeat_time):
        ...
        ########################### load checkpoint ###########################
        # state_dict = torch.load('/data/caidonghong/final_downstream/02GJX_to_05ZLH_2/checkpoint_40.pt', 'cpu')
        ...
        for i in range(checkpoint[0], checkpoint[1] + 5, 5):
            i = i if i < checkpoint[1] else checkpoint[1]
            ...
            print(index)
        best_index_arr.append(best_index)

        print(f"Best checkpoint is from: {best_file_path}")
        print(best_index)

    pre_arr = [index.micro_pre for index in best_index_arr if index]
    rec_arr = [index.micro_rec for index in best_index_arr if index]
    f1_arr = [index.micro_f1 for index in best_index_arr if index]
    f2_arr = [index.micro_f_d for index in best_index_arr if index]
    print('-' * 48)
