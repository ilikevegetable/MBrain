...
def train_step(data_loader,
    ...
    for step, full_data in enumerate(tqdm(data_loader, disable=args.tqdm_disable)):
        batch_data, _, time_shift_score = full_data
        ...
        #
        #     if args.graph_construct == 'noGraph':
        #         concat_f = gAR_f
        #         concat_b = gAR_b
        #     else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ssl_stage', action='store_false',
    ...
    parser.add_argument('--database_save_dir', type=str, default='/data/caidonghong/new_database2_no_filter/01TGX/',
    ...
    parser.add_argument('--data_save_dir', type=str, default='/data/eeggroup/new_data2/01TGX/',
    ...
    train_dataset, val_dataset = load_dataset(
        ...
        shared_encoder=True,
        n_process_loader=50,
        cal_timeShift_label=args.measure_steps,
    )
    ...
    ssl_criterion = UnsupervisedCriterion(
        n_predicts=args.n_predicts,
        dim_output_concat=256*2 if args.graph_construct != 'noGraph' else 256,
    ...
    ########################### load checkpoint ###########################
    # state_dict = torch.load('/data/caidonghong/nips22/02GJX_MBrain_Instant/checkpoint_50.pt', 'cpu')
    # ssl_model.load_state_dict(state_dict["sslModel"], strict=True)
    # ssl_criterion.load_state_dict(state_dict["sslCriterion"], strict=True)
    # replace_criterion.load_state_dict(state_dict["replaceCriterion"], strict=True)
    # timeShift_criterion.load_state_dict(state_dict["timeShiftCriterion"], strict=True)
    # start_epoch = 50
    # best_acc = state_dict["BestACC"]
    ...
    g_params = list(ssl_model.parameters()) + \
    ...
    for epoch in range(start_epoch, n_epoch):
        ...
        loc_logs_train, current_loss = \
            train_step(train_loader, ssl_model, replace_criterion, timeShift_criterion,
                       optimizer, args, epoch)

        loc_logs_val = \
            val_step(val_loader, ssl_model, replace_criterion, timeShift_criterion,
                     args, epoch)
        ...
        current_acc = float(loc_logs_val["repAcc_val"].mean() + \
                            loc_logs_val["tsAcc_val"].mean())
