...
class MBrain(nn.Module):
    def __init__(self,
        ...
        if graph_construct == 'mi':
            ...
            # predefined mean matrix of similarity
                self.mean_matrix = similarity_mean_seeg(
    ...
    def diffusion_module(self, after_gAR_batch, forward=True):
        ...
        if self.graph_construct == 'sample_from_distribution':
            ...
            for i in range(10):
                ...
                final_sample_weights.append(sample_weight)
            final_sample_weights = torch.stack(final_sample_weights, dim=0)
            sample_weight = torch.mean(final_sample_weights, dim=0)

            # New Version
            # normal_distribution = torch.abs(torch.randn_like(var_matrix, device=var_matrix.device))
            # sample_weight = self.mean_matrix + var_matrix * normal_distribution
            ...
            return after_gnn_batch, sample_weight
    ...
    def forward(self, batch_data, train_stage=True):
        ...
        if self.graph_construct != 'noGraph':
            if train_stage:
                if self.direction == 'single':
                    ...
                    gnn_batch, sample_weights = self.diffusion_module(after_gAR_batch[:, :, window_size - 1, :self.hidden_dim], True)
                    after_gnn_batch.append(gnn_batch)
        ...
        #
        #     return after_encoder_batch, after_gAR_batch, after_gnn_batch, sample_weights, reaplce_after_gAR, replace_label
        # else:
        return after_encoder_batch, after_gAR_batch, after_gnn_batch, sample_weights